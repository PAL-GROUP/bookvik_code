...
public class RenderTextureCamera : MonoBehaviour
{
    ...
    public  Camera Render_Texture_Camera;
    ...
    private IEnumerator StartRenderingToTexture()
    {
        ...
        print(Render_Texture_Camera.GetComponent<Camera>().targetTexture.width + " " +
              Render_Texture_Camera.GetComponent<Camera>().targetTexture.height);
    ...
    private IEnumerator ShowTextureOnGUI()
    {
        ...
        if (ShowTexture)
        {
            ...
            if (!DebugGUITexture)
            {
                ...
                DebugGUITexture.transform.position =
                    new Vector3((1.0f - (0.3f * GuiTextureAspect / 2)) - 0.1f * (1 / ARCamera_Camera.aspect), 0.25f, 0.0f);
            }
            else
            {
                float GuiTextureAspect = TextureResolutionX / (TextureResolutionY * ARCamera_Camera.aspect);
    ...
    private IEnumerator TakeScreen()
    {
        ...
        Texture2D FrameTexture = new Texture2D(CameraOutputTexture.width, CameraOutputTexture.height,
